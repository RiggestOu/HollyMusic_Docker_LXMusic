...
export function PlaylistDetailPage() {
  ...
  // 右键菜单执行「移出歌单」后会广播该事件，此处收到即刷新歌曲列表
  useEffect(() => {
    const onPlaylistChanged = (event: Event) => {
      const detail = (event as CustomEvent<{ playlistId?: number }>).detail
      if (detail?.playlistId === id) void reload()
    }
    window.addEventListener('playlist-changed', onPlaylistChanged)
    return () => window.removeEventListener('playlist-changed', onPlaylistChanged)
  }, [id, reload])
    ...
    <div className="p-6">
      {loading ? (
        ...
        <>
          ...
          {tracks.length > 0 ? (
            <SongList tracks={tracks} playlistId={id} />
          ) : (
