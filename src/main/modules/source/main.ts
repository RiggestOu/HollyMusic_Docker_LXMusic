...
import USER_SOURCE_RENDERER_EVENT_NAME from './rendererEvent/name'
...
export const getProxy = () => {
  ...
  const envProxy = (global.envParams?.cmdParams as any)?.['proxy-server']
  if (envProxy) {
...
export const createWindow = async(userApi: LX.UserApi.UserApiInfo) => {
  await closeWindow()
  dir ??= process.env.NODE_ENV !== 'production' ? webpackUserApiPath : path.join(__dirname, 'source')

  if (!html) {
    html = await fs.promises.readFile(path.join(dir, 'renderer/source.html'), 'utf8')
  }
  const preloadUrl = process.env.NODE_ENV !== 'production'
    ? `${path.join(__dirname, '../dist/source-preload.js')}`
    : `${path.join(__dirname, 'source-preload.js')}`
  ...
  browserWindow.on('ready-to-show', async() => {
    ...
    sendEvent(USER_SOURCE_RENDERER_EVENT_NAME.initEnv, { ...userApi, script: await getScript(userApi.id), proxy: getProxy() })
  })
}
