...
const matchInfo = (scriptInfo: string) => {
  ...
  for (const [key, len] of Object.entries(INFO_NAMES) as Array<[string, number]>) {
    infos[key] ||= ''
...
const parseScriptInfo = (script: string) => {
  ...
  if (!result) throw new Error('无效的自定义音源文件')

  let scriptInfo = matchInfo(result[0])
  scriptInfo.name ||= `source_${new Date().toLocaleString()}`
  return scriptInfo
}
...
export const importApi = async(scriptRaw: string): Promise<LX.UserApi.UserApiInfo> => {
  ...
  for (const api of userApis) {
    ...
    if (existingScript === script) {
      throw new Error(`导入失败，脚本内容与已有的音源「${api.name}」相同`)
    }
  }
  const apiInfo = {
    id: `user_source_${Math.random().toString().substring(2, 5)}_${Date.now()}`,
    ...scriptInfo,
